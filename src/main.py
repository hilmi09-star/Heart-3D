...
# Mode 5: Tulisan "HBD MyPrincess"
text_img_hbd = np.zeros((200, 800), dtype=np.uint8)
cv2.putText(text_img_hbd, "HBD MyPrincess", (50, 130), cv2.FONT_HERSHEY_SIMPLEX, 2.8, 255, 10, cv2.LINE_AA)
y_indices_hbd, x_indices_hbd = np.where(text_img_hbd > 0)
x_text_hbd = (x_indices_hbd - 400) / 70.0
y_text_hbd = -(y_indices_hbd - 100) / 70.0 
z_text_hbd = np.random.uniform(-0.1, 0.1, len(x_text_hbd))
pos_text_hbd = np.zeros((NUM_PARTICLES, 3))
text_points_hbd = np.stack((x_text_hbd, y_text_hbd, z_text_hbd), axis=-1)
chosen_indices_hbd = np.random.choice(len(text_points_hbd), NUM_PARTICLES)
pos_text_hbd = text_points_hbd[chosen_indices_hbd]
...
def hitung_mode_gestur(hand_landmarks):
    ...
    if jari_berdiri[0] and jari_berdiri[1] and jari_berdiri[2] and not jari_berdiri[3]:
        return 5  # 3 Jari -> Teks HBD MyPrincess
...
def camera_thread_func():
    print("[INFO] Memulai koneksi ke kamera...")
    cap = cv2.VideoCapture(0)
    
    # Jika index 0 gagal, coba index lain (misal 1 atau 2)
    if not cap.isOpened():
        print("[WARNING] Kamera pada indeks 0 tidak dapat dibuka. Mencoba indeks cadangan...")
        for i in [1, 2, -1]:
            cap = cv2.VideoCapture(i)
            if cap.isOpened():
                print(f"[SUCCESS] Berhasil terhubung ke kamera pada indeks: {i}")
                break
        else:
            print("[ERROR] Tidak dapat membuka kamera apa pun! Pastikan kamera Anda terhubung dan tidak sedang digunakan oleh aplikasi lain.")
    else:
        print("[SUCCESS] Berhasil terhubung ke kamera pada indeks 0.")

    # Trik khusus untuk memastikan kamera benar-benar siap sebelum memulai loop utama 
    cv2.waitKey(500)
    ...
    cap.release()
    print("[INFO] Koneksi kamera ditutup.")

# Jalankan Thread Kamera terlebih dahulu
camera_thread = threading.Thread(target=camera_thread_func, daemon=True)
camera_thread.start()

# Berikan jeda 1 detik agar kamera siap sebelum Pygame mengambil alih GUI utama
time.sleep(1.0)
...
pygame.display.set_caption("Space Gesture Controller - NebulaHeart 3D")
